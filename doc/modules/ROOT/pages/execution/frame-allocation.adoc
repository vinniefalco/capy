...
By default, `run_async` uses a recycling frame allocator that caches deallocated
...
== Custom Allocators with run_async

Pass a custom allocator as the second argument to `run_async`:
...
The `run_async` function manages this automatically—you rarely need to call
these directly.
